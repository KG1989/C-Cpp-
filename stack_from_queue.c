...
{
     //FIXME: you must write this
	struct listQueue *newQueue = (struct lisQueue *) malloc(sizeof(struct listQueue));
	assert(newQueue != 0);
...
int listQueueIsEmpty(struct listQueue *q) {
    ...
    //FIXME: you must write this
	//I think I could have also done if (q->firstLink->next == NULL)
	if (q->lastLink == q->lastLink)
	{
		return 1;
	}
	else
...
void listQueueAddBack(struct listQueue *q, TYPE e) {
	...
	newLink->value = e;
	//newLink is NULL cus adding to the end
	newLink->next = NULL;

	if (q->firstLink->next == NULL)
	{
		//if list is empty, firstLink->next should be the newLink
		q->firstLink->next = newLink;
	}

	if (q->lastLink != q->firstLink)
	{
...
int main(int argc, char* argv[])
{
	...
	/* debugging
	struct listQueue *queue = listQueueCreate();

	printf("testing queue add operation...\n");

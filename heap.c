...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    int lessValueChild = position;

    if (rightChild < last)
    {
        if (compare(dyGet(heap, leftChild), dyGet(heap, rightChild)) == 1)
        {
            lessValueChild = rightChild;
        }
        else
        {
            lessValueChild = leftChild;
        }

        if (compare(dyGet(heap, lessValueChild), dyGet(heap, position)) == -1)
        {
            dySwap(heap, lessValueChild, position);
            adjustHeap(heap, last, lessValueChild, compare);
        }
    }
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
    // FIXME: implement
    dyAdd(heap, value);
    //current place where value is added
    int current = heap->size - 1, parent;
    
    while (current > 0)
    {
        parent = (current - 1) / 2;
        if(compare(dyGet(heap, current), dyGet(heap, parent)) == -1) //less than
        {
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
    ...
    //call adjust heap (dynArray, last, position, compare)
...
TYPE dyHeapGetMin(DynamicArray* heap)
{
    // FIXME: implement
    assert (heap != NULL);
    //use dyGet function, passing in heap and position 0
    return dyGet(heap, 0);
    //return NULL;
}
